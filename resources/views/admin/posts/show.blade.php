            ...
            <div class="p-6">
                <div class="flex justify-between items-start">
                    ...
                    <div class="flex space-x-2">
                        @if ($post->is_pinned)
                            <span class="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm">Pinned</span>
                        @endif
                        @if ($post->is_locked)
                ...
                <div class="mt-6 flex space-x-2">
                    ...
                    <form method="POST" action="{{ route('admin.posts.pin', $post) }}">
                        @csrf
                        <button type="submit"
                            class="bg-{{ $post->is_pinned ? 'gray' : 'green' }}-600 hover:bg-{{ $post->is_pinned ? 'gray' : 'green' }}-700 text-white px-4 py-2 rounded">
